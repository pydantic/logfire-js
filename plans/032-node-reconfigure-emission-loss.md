...
- **Mixed OTel ownership**: if the application registered its own global tracer provider before logfire's first `configure()`, logfire's registration already fails silently today; with this change, a _re_-configure will disable the app's globals and take them over (last-configure-wins). Bounded: such setups are already broken today (logfire emissions never worked there), and the registrant marker limits disables to runtimes logfire started. User-accepted 2026-07-15 (see Clarifications): implement last-configure-wins without external-owner detection.
