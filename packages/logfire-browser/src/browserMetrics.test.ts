...
describe('browser metrics runtime', () => {
  ...
  it('keeps a metric attribute named like an Object.prototype member', async () => {
    const runtime = await startBrowserMetrics({ metricUrl: '/v1/metrics/browser' }, { attributes: {} } as never)
    const recorder = runtime.createWebVitalsMetricRecorder({
      // Built by entries: an object literal `__proto__` key would set the prototype, not a key.
      attributes: () =>
        Object.fromEntries([
    ...
    // A plain record write to `__proto__` runs the inherited setter and drops the entry.
