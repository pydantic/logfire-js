 ...
 */
type ErrorBrand<T extends IndexType> = Record<T, void>
...
type StateMachineDefinition<S, A> = {
  handlers: Record<string, Record<string, (cur: MapValues<S>, action: MapValues<A>) => MapValues<S>>>
}
...
type ActionHandlerCallback<States, Transitions, CS extends StateType, AN extends ActionNameType, NS extends MapValues<States>, Actions> = (
...
const actionHandler = <StateMap, Transitions, ActionMap, HandledStates>(
  ...
  return <S extends StateType, AN extends ActionNameType, NS extends MapValues<StateMap>>(
    ...
    const handlers = definition.handlers as Record<string, Record<string, unknown> | undefined>
    const newDefinition: StateMachineDefinition<StateMap, ActionMap> = {
      ...definition,
      handlers: {
        ...definition.handlers,
        [untypedState]: {
          ...(handlers[String(untypedState)] ? handlers[String(untypedState)] : {}),
