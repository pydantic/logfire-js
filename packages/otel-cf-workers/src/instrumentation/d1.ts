...
function instrumentD1StatementFn(fn: Function, dbName: string, operation: string, sql: string) {
  ...
  const fnHandler: ProxyHandler<any> = {
    apply: (target, thisArg, argArray) => {
      if (operation === 'bind') {
        const newStmt: D1PreparedStatement = Reflect.apply(target, thisArg, argArray)
...
function instrumentD1PreparedStatement(stmt: D1PreparedStatement, dbName: string, statement: string): D1PreparedStatement {
...
export function instrumentD1Fn(fn: Function, dbName: string, operation: string): Function {
  const tracer = trace.getTracer('D1')
  const fnHandler: ProxyHandler<any> = {
    apply: (target, thisArg, argArray) => {
      if (operation === 'prepare') {
        const sql = argArray[0] as string
        const stmt: D1PreparedStatement = Reflect.apply(target, thisArg, argArray)
        ...
        return tracer.startActiveSpan(`${dbName} ${operation}`, async (span) => {
          ...
          try {
            const result: D1Result[] = await Reflect.apply(target, thisArg, argArray)
            result.forEach((r: D1Result, i: number) => subSpans[i]?.setAttributes(metaAttributes(r.meta)))
            span.setStatus({ code: SpanStatusCode.OK })
            return result
          } catch (error) {
            span.recordException(error as Exception)
