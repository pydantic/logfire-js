...
function endSpan(currentState: InProgressTraceState, { span }: EndSpanAction): InProgressTraceState | TraceCompleteState {
  ...
  private export(localRootSpanId: string) {
    ...
    if (newState.stateName === 'exporting') {
      ...
      promise.then(
        ...
        (error: unknown) => {
          console.log('Error sending spans to exporter:', error)
          this.action(localRootSpanId, { actionName: 'exportDone' })
          this.inprogressExports.delete(localRootSpanId)
        }
      )
