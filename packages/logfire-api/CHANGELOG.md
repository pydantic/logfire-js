...
- ef5e6e3: Carry a task's own return value through the evals output seams. `renderReport` crashed the whole report when a case's output or inputs held a BigInt (which `JSON.stringify` throws on) or was `undefined` (which it refuses to stringify); `recordReturn` lost the entire return value to one BigInt field. Both now serialize under the same replacer the attribute seam applies, so a BigInt survives as an exact number or decimal string and a `void` task renders as `undefined` in its cell.
- 7ffee54: Keep both ends when truncating a message field or baggage value. `truncateString` kept only the head, but the tail is usually what tells two long values apart — an asset hash at the end of a URL, an ID at the end of a path — and Python's `truncate_string` already keeps both halves around a middle ellipsis for the same fields. Both cuts respect surrogate pairs.
