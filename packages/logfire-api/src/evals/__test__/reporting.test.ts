...
describe('report-level evaluators land on the experiment span', () => {
  ...
  it('counts a label value of __proto__ like any other', () => {
    ...
    // leaving the surviving one at 100%.
    // The expected distribution is built with `fromEntries` for the same reason: a `__proto__`
    // key in an object literal sets the prototype instead of becoming a key.
