...
`@pydantic/logfire-browser` does not read Logfire credentials from environment variables. Pass the regional URL and restricted frontend application token to `configureFrontend({ baseUrl, token })`. Use a backend proxy only if browser traffic needs application-specific authentication, origin restrictions, or rate limits.
